...
#New in v 2.4.2
# added Rx_N pfam domain to the list of canonical R gene domains 
...
#		    unless ($domain=~ m/(/) die "Looks like pfamscan file does not contain verbose information for each domain\n"; 
			...
			unless ($domainid=~ m/NB-ARC|LRR|AAA|AAA.+|TIR|RPW8|Rx_N/ ){ #skip known domains
	...
	unless ( @{$sd_domains{'family'}{$family}} ){

	    @{$sd_domains{'family'}{$family}} = '';
	}

        unless ( @{$sd_domains{'species'}{$species}} ){
